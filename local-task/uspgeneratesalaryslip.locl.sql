...
2.1		13-Oct-2025		Add Overtime										Shiv Kumar
2.2 	04-Dec-2025		Bifurcate Vouchers and Deductions					Shiv Kumar
*************************************************************************************************************/
declare
...
(
	...
	overtime NUMERIC(18,2),
	arr_overtime NUMERIC(18,2),
	salaryjson text,
	salid1 bigint,
	salid2 bigint,
	salid3 bigint
...
select v_rec.mon,v_rec.employee,v_rec.employee_name,v_rec.father_husband_name,v_rec.designation,v_rec.department,v_rec.
	...
	v_rec.overtime,v_rec.arr_overtime,
	v_rec.salaryjson
...
round(max(tmprate.fixedallowancestotalrate) + case when v_customeraccountid=981 then coalesce(max(tmprate.ratetransport_allowance),0) + coalesce(max(tmprate.ratetravelling_allowance),0) else 0 end) rategross,
...
,round(coalesce(sum(overtime),0)::numeric(18,2))	overtime
,round(coalesce(sum(arr_overtime),0)::numeric(18,2)) overtime_arear
