...
v_allowed_customer_accounts bigint[] := ARRAY[981];
begin
/**********************************************************************************************
...
1.4		13-Oct-2025		Add Overtime												Shiv Kumar
...
,coalesce(t1.overtime,0.0) overtime
,coalesce(t2.arr_overtime,0.0)+coalesce(t3.arr_overtime,0.0) as arr_overtime
,nullif(t1.salaryjson,'') salaryjson,
t1.id as salid1,
t2.id as salid2,
t3.id as salid3
-- case when coalesce(t1.otherledgerdeductions,0.0) <>0 then t1.disbursedledgerids else null end as otherledgerids1,
-- case when coalesce(t2.otherledgerdeductions,0.0) <>0 then t2.disbursedledgerids else null end as otherledgerids2,
-- case when coalesce(t3.otherledgerdeductions,0.0) <>0 then t3.disbursedledgerids else null end as otherledgerids3
...
left join banktransfers 
	 ...
	 and (t1.issalaryorliability='S' or (v_customeraccountid = ANY(v_allowed_customer_accounts) and t1.issalaryorliability='L'))
...
where tbl_monthlysalary.disbursementmode=p_disbursementmode
	 and (tbl_monthlysalary.issalaryorliability='S' or (v_customeraccountid = ANY(v_allowed_customer_accounts) and tbl_monthlysalary.issalaryorliability='L'))
