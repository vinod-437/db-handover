...
1.8			01-Sep-2023		Add Limits as per monthly salary and  	Shiv Kumar
							Wallet Amount
1.9			01-Sep-2023		Add p_is_reimbursement_claim,  			Parveen Kumar
							and p_reimbursement_claim_id
1.10		31-Aug-2024		Add p_voucher_source p_unique_bulk_voucher_batch_id  	Parveen Kumar
1.11		19-Feb-2025		Populate Monthwise Investment		Shiv Kumar
1.12		21-Mar-2025		Independent Contractors	10% TDS			Shiv Kumar
1.13		15-Nov-2025		TDS	in Current/Next month				Shiv Kumar
1.14        21-Feb-2026     Leave Encashment Id added               Rekha
1.15        07-Apr-2026     Bonus id added							Rekha
*************************************************************************/
...
-- 	end if;
	...
	select (coalesce(tblrec.netamountreceived,0)-coalesce(tblpay.netamountpaid,0))::numeric(18,2) barebalance
		...
		left join (
			...
			sum((ts.grossearning+coalesce(ts.employeresirate)+coalesce(ts.ac_1,0)+coalesce(ts.ac_10,0)+coalesce(ts.ac_2,0)+coalesce(ts.ac21,0))*(case when coalesce(tl.headid,0)<>67 then 1 when mprmonth=p_mprmonth and mpryear=p_mpryear then  100.0/75.0 else 0.0 end)) netamountpaid
			from openappointments op inner join tbl_monthlysalary ts
			on op.emp_code=ts.emp_code
			and ts.is_rejected='0'
			and op.recordsource='HUBTPCRM'
...
INSERT INTO public.cmsdownloadedwages(
	...
	,case when (p_loanmasterid is null OR p_fnfid is not null) then 
...
/*******************************************/
	...
	(
		...
		is_reimbursement_claim, reimbursement_id,
		voucher_source, voucher_source_batch,tdsdeductionmonth
		,assetid,fnfid,encashment_id, bonus_id
	)
	select
	coalesce(p_emp_id,v_emp_id),p_emp_code,headid,headname,amount,p_mprmonth,p_mpryear,
	'1',p_createdby,current_timestamp,p_createdbyip ,masterheadname,is_taxable,nullif(p_loanmasterid,-9999),is_billable,p_remarks,nullif(p_createdbyusertype,''),
		p_is_reimbursement_claim, p_reimbursement_claim_id,
	p_voucher_source, p_unique_bulk_voucher_batch_id,p_tdsdeductionmonth
	,nullif(p_assetid,-9999),p_fnfid,p_encashment_id,p_bonus_id
	from tmp_ledgers
...
/*****************************************************/	
	...
	if coalesce(p_tdsdeductionmonth,'current')<>'next' then
		...
		raise notice 'p_year=>%',(case when p_mprmonth in (1,2,3) then p_mpryear-1 else p_mpryear end);
			...
			/*	 -- if we get the fnf the set tds =0 dated. 19.04.2026
				 IF p_fnfid is not null THEN
					v_tds := 0;
				 end if;
...
/************************************************************/
		...
		if v_amount>0 then
			if v_headid=9 or v_headid=67 then
	...
	with v_tbl_monthlysalary as (
		...
		0 insurance, 
		0 mobile, 
		case when v_headid in(19,68) then v_otherledgerdeductions  when v_headid in(9,67) then v_advance else 0 end advance, 
		0 other, 
		...
		v_otherledgerarears otherledgerarears,case when v_headid in(19,39,10,68) then  0 else v_otherledgerdeductions end otherledgerdeductions,
