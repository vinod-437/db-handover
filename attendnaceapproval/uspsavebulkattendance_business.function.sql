...
return msg; --v_cnt::text||' records have been approved successfully.';
	...
	if p_action='SaveBulkAttendance' or  p_action='ApproveBulkAttendance' or p_action='LockAttendance'  then
		...
		if not EXISTS (SELECT * FROM pg_catalog.pg_class c   JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace WHERE  c.relname = 'tmpbulkattendance' AND c.relkind = 'r' and n.oid=pg_my_temp_schema()) then
			...
			delete from tmpbulkattendance;
		end if;

		if not EXISTS (SELECT * FROM pg_catalog.pg_class c   JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace WHERE  c.relname = 'tmpresponse' AND c.relkind = 'r' and n.oid=pg_my_temp_schema()) then		
		...
		if p_action='SaveBulkAttendance' then
			...
			*/
			...
			/*****************************Change 2.4 starts*****************************************************/		
			if (p_att_purpose='Attendance'::text 
					and exists(
							select * from tbl_monthly_attendance
							where tbl_monthly_attendance.att_date between greatest(v_rec_payrolldates.start_dt,v_openappointments.dateofjoining) and least(v_rec_payrolldates.end_dt,coalesce(v_openappointments.dateofrelieveing,v_rec_payrolldates.end_dt))
										and tbl_monthly_attendance.emp_code=p_emp_code
										AND (tbl_monthly_attendance.attendance_salary_status = '1' AND tbl_monthly_attendance.is_attendance_salary = 'Salary')
										and tbl_monthly_attendance.approval_status='A'
							)
				   ) then
				null;   
			else
...
if p_action='ApproveDailyAttendance' then
		...
		if not EXISTS (SELECT * FROM pg_catalog.pg_class c   JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace
    		WHERE  c.relname = 'tmpbulkattendance' AND c.relkind = 'r' and n.oid=pg_my_temp_schema()
					  ) then
		create temporary table tmpbulkattendance 
		(
			attendancedate text,
			...
			if not EXISTS (SELECT * FROM pg_catalog.pg_class c   JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace
    		WHERE  c.relname = 'tmpresponse' AND c.relkind = 'r' and n.oid=pg_my_temp_schema()
					  ) then		
		create temporary table tmpresponse
		(
			att_date date,
