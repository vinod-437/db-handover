...
return next v_refsalaryslip;
	...
	RAISE NOTICE 'v_tptype :: %', v_tptype;
/********************************|| START - Parveen on 15 May 2023||********************************/
...
RAISE NOTICE 'v_principal_employer_name :: %', v_principal_employer_name;
RAISE NOTICE 'v_customeraccountid :: %', v_customeraccountid;
...
-- START - Change [1.6]
	...
	-- START - Change [2.4] - Moved OPEN v_refsalaryslip logic to line 518 to prevent duplicate cursor error
	-- We will evaluate this condition later at line 518 to prevent duplicate cursor OPEN
	-- END - Change [2.4]
-- END - Change [1.6]
...
RAISE NOTICE 'v_employername :: %', v_employername;
/*********************************************************************************/
...
and is_rejected<>'1';
RAISE NOTICE 'v_paiddays :: %', v_paiddays;
RAISE NOTICE 'v_monthdays :: %', v_monthdays;
RAISE NOTICE 'v_lopdays :: %', v_lopdays;
/*********************Change 1.8 starts**********************/
...
RAISE NOTICE 'v_lrelievingdays :: %', v_lrelievingdays;
RAISE NOTICE 'v_lopdays :: %', v_lopdays;
/*************88Change 1.8 ends********************************/
...
-- START - Change [2.4] - Moved from line 140 to prevent duplicate cursor error
or (v_view_salary_slip = 'Y' AND p_origin = 'Employee')
-- END - Change [2.4]
) then
...
open v_refsalaryslip for
	with tmp_jobrole as (
		...
		where mprmonth=p_month	 and mpryear=p_year and empcode=p_emp_code::text
			...
			or (v_payoutmodetype in ('self','hybrid','EOR') and
			(cmsdownloadedwages.empcode::bigint,cmsdownloadedwages.batch_no,cmsdownloadedwages.mprmonth,cmsdownloadedwages.mpryear)
				in (
...
--raise notice 'disbursedledgerids=>%',(select disbursedledgerids from tmpsalslip);
