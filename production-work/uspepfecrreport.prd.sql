...
CREATE OR REPLACE FUNCTION public.uspepfecrreport_temp(
	p_rpt_month integer,
	p_rpt_year integer,
...
1.7 		14-Aug-2025		Reduce Joining Days from NCP days	Shiv Kumar
							in EPF Report
1.8 		11-May-2026		Added support for -9998 			AI Assistant-vinod
							customeraccountid to filter DFM 
							Auto Paid accounts
**************************************************************************/
...
select extract ('day' from make_date(p_rpt_year,p_rpt_month,1)+interval '1 month -1 day') into v_totalmonthdays;
		...
		v_reportquery:='select o.emp_id appointment_id,'||p_rpt_month||' rpt_month,'||p_rpt_year||' rpt_year,	
			o.uannumber uan,o.emp_name member_name,o.emp_code,e.batchid,o.cjcode tpcode,o.orgempcode,o.cjcode,';
		if p_reporttype<>'LWF' then		
...
union all
	...
	------------------------------------------------
			...
			-- OLD CODE (11-May-2026): and o.customeraccountid=coalesce(nullif('||p_customeraccountid||',''-9999'')::bigint,o.customeraccountid)
			and (
				('''||p_customeraccountid||''' = ''-9998'' and o.customeraccountid in (select a.customeraccountid from tbl_receivables a where a.paymentmethod = ''DFM Auto'' and a.status = ''Paid''))
				OR ('''||p_customeraccountid||''' = ''-9999'')
				OR ('''||p_customeraccountid||''' not in (''-9999'', ''-9998'') and o.customeraccountid = '''||p_customeraccountid||'''::bigint)
			)
			...
			(
				SELECT 1
				FROM unnest(string_to_array(COALESCE(NULLIF('''||coalesce(p_ou_ids,'')||''', ''''), COALESCE(NULLIF(o.assigned_ou_ids, ''''), COALESCE(NULLIF(o.geofencingid::TEXT, ''''), ''0''))), '','')) AS input_ou_ids
			...
			execute 'create temporary table  tmp_challan ON COMMIT DROP as '|| v_reportquery ;
			open rfcreport for
				select * from tmp_challan;
			return rfcreport;
		end if;
...
--				case when coalesce(o.epf_pension_opted,''Y'')=''Y'' then (epf_eps_diff_remitted+(greatest(eps_contri_remitted-least(eps_contri_remitted,1250),0)))::numeric(18,2) else round((coalesce(epf_eps_diff_remitted,0)+coalesce(eps_contri_remitted,0))::numeric(18,2),2) end  epf_eps_diff_remitted,
			...
			v_reportquery:=v_reportquery
			||coalesce(p_downloadedby::text,'null')||','''
			||v_reporttimestamp||''','''
			||coalesce(p_downloadedbyip,'')||''',
...
---------------------------------------------------------------------------		
	...
	------------------------------------------------
			...
			*/
			...
			-- OLD CODE (11-May-2026): and op.customeraccountid=coalesce(nullif(p_customeraccountid,'-9999')::bigint,op.customeraccountid) -- SIDDHARTH BANSAL 13/03/2024
			and (
				(p_customeraccountid = '-9998' and op.customeraccountid in (select a.customeraccountid from tbl_receivables a where a.paymentmethod = 'DFM Auto' and a.status = 'Paid'))
				OR (p_customeraccountid = '-9999')
				OR (p_customeraccountid not in ('-9999', '-9998') and op.customeraccountid = p_customeraccountid::bigint)
			) -- SIDDHARTH BANSAL 13/03/2024
		...
		if p_action='RetrieveProcessedReport' then
			...
			-- OLD CODE (11-May-2026): and op.customeraccountid=coalesce(nullif(p_customeraccountid,'-9999')::bigint,op.customeraccountid)
			and (
				(p_customeraccountid = '-9998' and op.customeraccountid in (select a.customeraccountid from tbl_receivables a where a.paymentmethod = 'DFM Auto' and a.status = 'Paid'))
				OR (p_customeraccountid = '-9999')
				OR (p_customeraccountid not in ('-9999', '-9998') and op.customeraccountid = p_customeraccountid::bigint)
