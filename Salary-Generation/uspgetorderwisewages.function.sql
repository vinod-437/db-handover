...
1.42	24-June-2025	corss Month changes 				Vinod Kumar
1.43	04-Jul-2025		Meal Voucher						Shiv Kumar
1.43	01-Aug-2025		Save Meal Voucher					Shiv Kumar
1.44	23-Sep-2025		Adding Gross and grossearnin	Shiv Kumar
1.45	02-Dec-2025		Add flexiblemonthdays				Shiv Kumar
...
v_querytext:=v_querytext||'emp_code,bunit subunit,dateofleaving,totalleavetaken,
	...
	((coalesce(grossearningcomponents,FixedAllowancesTotal,0))+(coalesce(FixedAllowancesTotalRate_arr,0))+
		...
		+(coalesce(km_distance,0)*coalesce(perkilometerrate,0))+coalesce(tea_allowance,0))
		-( (case when epf>0 then
		case when coalesce(pfcapapplied,''Y'')=''N'' then pfapplicablecomponents*12.0/100 
			else
				case when (pfapplicablecomponents+coalesce(pfapplicablecomponentsalreadypaid,0))<15000  then pfapplicablecomponents*12.0/100 
...
((coalesce(nullif(esiapplicablecomponents,0),coalesce(basic,0)+coalesce(hra,0)+coalesce(SpecialAllowance,0)+coalesce(Medical,0)+coalesce(commission,0)+coalesce(transport_allowance,0)+coalesce(overtime_allowance,0)+coalesce(children_education_allowance,0)+'|| coalesce(p_paymentadvice.otherearningcomponents,0)||')+'||v_overtime||'+(coalesce(km_distance,0)*coalesce(perkilometerrate,0)))*0.03250)
...
,ratesalarybonus
,ratecommission
,coalesce(ratetransport_allowance,0)+(coalesce(km_distance,0)*coalesce(perkilometerrate,0)) ratetransport_allowance
,ratetravelling_allowance
...
,salarybonus
,commission
,coalesce(transport_allowance,0)+(coalesce(km_distance,0)*coalesce(perkilometerrate,0)) transport_allowance
...
if p_action='Retrieve_Salary' then	
		...
		employeeesirate,employeresirate,
	        ...
	        0::numeric(18,2) lwf,
			case when openappointments.customeraccountid in(6927,7416,6533) then (coalesce(insuranceamount,0)+coalesce(familyinsuranceamount,0))-coalesce(alreadyinsurance,0) else
		...
		,tblotherdeductions.otherdeductions*(case when coalesce(salaryindaysopted,''N'')=''N'' or empsalaryregister.salarydays=1 then (totalpaiddays+totalleavetaken) else least(salarydays,(totalpaiddays+totalleavetaken)) end)/case when coalesce(salaryindaysopted,''N'')=''N'' or '''||coalesce(v_empsalaryregister.flexiblemonthdays,'N')||'''=''Y'' then '||v_monthdays||'  else salarydays end otherdeductions
									...
									case when openappointments.customeraccountid in(6927,7416,6533) then coalesce(insuranceamount,0)-coalesce(alreadyinsuranceamount,0) else
									...
									case when openappointments.customeraccountid in(6927,7416,6533) then coalesce(familyinsuranceamount,0)-coalesce(alreadyfamilyfamilyinsurance,0) else
	...
	,case when openappointments.customeraccountid in(6927,7416,6533) then coalesce(employerinsuranceamount,0)-coalesce(alreadyemployerinsuranceamount,0) else
...
union all				  
				select salaryid,candidate_id,sum(deduction_amount)*1 otherdeductions
				  ...
				  and motd.id<>134 --Meal Voucher ID
					--and '||v_openappointments.customeraccountid||' not in(@customerid)
				  group by salaryid,public.trn_candidate_otherduction.candidate_id  
				  ) tblotherdeductions
...
----------------------------------------------------------------------	
	...
	,string_agg(deduction_name||'':''||deduction_amount,'','') customnontaxablecomponents
				  ...
				  and trn_candidate_otherduction.deduction_frequency in (''Monthly'')
				  and motd.id<>134 --Meal Voucher ID, Must Change for Production
				  and trn_candidate_otherduction.deduction_id not in (5,6,7,10)
				  and coalesce(trn_candidate_otherduction.is_taxable,''N'')=''N''
