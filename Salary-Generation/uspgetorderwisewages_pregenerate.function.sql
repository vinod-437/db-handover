...
((coalesce(nullif(esiapplicablecomponents,0),coalesce(basic,0)+coalesce(hra,0)+coalesce(SpecialAllowance,0)+coalesce(Medical,0)+coalesce(commission,0)+coalesce(transport_allowance,0)+coalesce(overtime_allowance,0)+coalesce(children_education_allowance,0))+'||v_overtime||'+(coalesce(km_distance,0)*coalesce(perkilometerrate,0)))*0.03250)
...
v_querytext:=v_querytext||'	inner join 
	...
	/*and coalesce(cmsdownloadedwages.salary_current_advance,''Current'')<>''Advance''*/ ';
