...
BEGIN
	...
	SELECT string_agg(jobtypecode, ',') INTO v_jobtypecode FROM mst_jobtype WHERE jobtypecode <> 'Meeting';
	...
	ELSIF p_action = 'GetAttendanceDetailByemployee' THEN
		OPEN v_result FOR
			...
			SELECT
				...
				-- START - Changes [3.8]
				TO_CHAR(CAST(tma.latehours AS TIME), 'HH24:MI') late_check_in_time, tma.latehoursdeduction late_check_in_amount,
				TO_CHAR(CAST(tma.earlyhours AS TIME), 'HH24:MI') early_check_out_time, tma.earlyhoursdeduction early_check_out_amount,
				-- END - Changes [3.8]
				TO_CHAR(CAST(tma.overtime_hours_approved_by_employer AS TIME), 'HH24:MI') overtime_hours_approved_by_employer, tma.overtime_amount_approved_by_employer overtime_amount_approved_by_employer, -- END - Changes [3.9]
	...
	ELSIF p_action = 'GetDailyAttendanceDetail' THEN
		OPEN v_result FOR
			SELECT
				...
				NULLIF(tc.document_path,'https://api.contract-jobs.com/crm_api/') AS photopath,
				op.orgempcode, op.cjcode tpcode,
				tma.is_overtime is_overtime_applicable, TO_CHAR(CAST(tma.no_of_overtime_hours_worked AS TIME), 'HH24:MI') no_of_overtime_hours_worked,
				tma.deviation_in_checkin, tma.deviation_in_checkout, tma.deviation_in_total_working_hours,
				...
				TO_CHAR(CAST(tma.latehours AS TIME), 'HH24:MI') late_check_in_time, tma.latehoursdeduction late_check_in_amount,
				TO_CHAR(CAST(tma.earlyhours AS TIME), 'HH24:MI') early_check_out_time, tma.earlyhoursdeduction early_check_out_amount,
				TO_CHAR(CAST(tma.overtime_hours_approved_by_employer AS TIME), 'HH24:MI') overtime_hours_approved_by_employer, tma.overtime_amount_approved_by_employer overtime_amount_approved_by_employer,
				leavetype,
